...
if(sys.argv[1] == "bup"):
    ...
    cmd = "dunstify -r 1234 -t 2000 \'Brightness : "+str(percent)+" %\'"
    print(cmd)
...
if(sys.argv[1] == "vup"):
    if(getvol()<150):
        ...
        percent = int(percent)
        if(percent<=100):
            cmd = "dunstify -r 1234 -t 2000 \'Volume : "+str(percent)+" %\'"
            subprocess.run(cmd,shell=True,capture_output=True,text=True)
        elif(percent>100 and percent<=150):
            cmd = "dunstify -u critical -r 1234 -t 2000 \'Volume : "+str(percent)+" %\'"
            subprocess.run(cmd,shell=True,capture_output=True,text=True)
...
if(sys.argv[1] == "rofi"):
    ...
    if(result.stdout=="SLEEP\n"):
        subprocess.run("hyprlock & systemclt suspend",shell=True)
